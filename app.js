...
// ════════════════════════════════════════════════════════
// ADMIN-PIN (bytt denne i produksjon)
// Lagres ikke i Firebase — kun i applikasjonslaget.
// ════════════════════════════════════════════════════════
...
try {
  ...
  visFBFeil('Kunne ikke koble til Firebase: ' + (e?.message ?? e));
}
...
// Fast americano-rotasjon for 5-spillerbaner (A=0,B=1,C=2,D=3,E=4)
// Alle spiller med og mot alle over 5 runder — én hviler per kamp.
const PARTER_5 = [
  { nr:1, lag1:[0,1], lag2:[2,3], hviler:4 }, // A+B vs C+D, E hviler
  { nr:2, lag1:[0,2], lag2:[1,4], hviler:3 }, // A+C vs B+E, D hviler
  { nr:3, lag1:[0,3], lag2:[2,4], hviler:1 }, // A+D vs C+E, B hviler
  { nr:4, lag1:[0,4], lag2:[1,3], hviler:2 }, // A+E vs B+D, C hviler
  { nr:5, lag1:[1,2], lag2:[3,4], hviler:0 }, // B+C vs D+E, A hviler
];

// Parter for dobbel-bane i 6-spiller-format (kun 1 kamp per runde)
const PARTER_6_DOBBEL = [
  { nr:1, lag1:[0,1], lag2:[2,3], singel:false },
];
// Parter for singel-bane i 6-spiller-format (kun 1 kamp per runde)
const PARTER_6_SINGEL = [
  { nr:1, lag1:[0], lag2:[1], singel:true },
];

// ════════════════════════════════════════════════════════
// 6-SPILLER FORMAT — 2 baner per runde:
//   Bane 1: Dobbel (4 spillere)
//   Bane 2: Singel (2 spillere)
//
// Rotasjonsregler:
//   1. Taperlaget → singel neste runde
//   2. Vinnerlaget splittes
//   3. Singel-spillerne inn på dobbel
//   4. Rating-basert lagsetting (snake-draft):
//      høyest vinner + lavest singel vs lavest vinner + høyest singel
// ════════════════════════════════════════════════════════

/**
 * Genererer neste runde for 6-spiller-format basert på kampresultat og rating.
 *
 * @param {Object} dobbelKamp      — { lag1: [spiller,spiller], lag2: [spiller,spiller], vinnerId: 1|2|null }
 * @param {Array}  singelSpillere  — [spiller, spiller] fra singelbanen
 * @returns {{ dobbelLag1: [spiller,spiller], dobbelLag2: [spiller,spiller], singelPar: [spiller,spiller] }}
 *
 * Lagsetting (snake-draft):
 *   Sorter vinnere etter rating høy→lav: [vHøy, vLav]
 *   Sorter singel-spillere etter rating høy→lav: [sHøy, sLav]
 *   Lag 1: vHøy + sLav  (høyest vinner pares med lavest singel)
 *   Lag 2: vLav + sHøy  (lavest vinner pares med høyest singel)
 */
function neste6SpillerRunde(dobbelKamp, singelSpillere) {
  const { lag1, lag2, vinnerId } = dobbelKamp;

  // Bestem vinner- og taperlag (ved uavgjort: lag1 som vinner)
  const vinnere = vinnerId === 2 ? lag2 : lag1;
  const tapere  = vinnerId === 2 ? lag1 : lag2;

  // Taperlaget → singel neste runde (rekkefølge spiller ingen rolle)
  const singelPar = tapere;

  // Snake-draft: sorter vinnere og singel-spillere etter rating
 ...
 *   13 → [5,4,4]
 *
 * @param {Array}  spillere   — array av { id, navn, rating }
 * @param {number} antallBaner — ønsket antall baner fra oppsett
 * @returns {Array} [{ baneNr, spillere }]
 */
function fordelBaner(spillere, antallBaner) {
  ...
  // Nøyaktig 6 spillere og 2 baner: bane 1 dobbel (4 spl), bane 2 singel (2 spl)
  // Bruker greedy-algoritmen fra null-state for å velge hvem som starter i singel
  if (n === 6 && antallBaner === 2) {
    const mp = app.poengPerKamp ?? 15;
    // Runde 1: topp 2 og bunn 2 på dobbel, midterste 2 på singel (rating-sortert)
    // [0]=høyest, [5]=lavest → singel: [2],[3] — dobbel: [0],[1],[4],[5]
    // Snake-draft på dobbel: lag1=[0,5], lag2=[1,4]
  ...
  // Antall 5-spillerbaner = rest av n % 4
  // Resten får 4. Dette gir minst mulig 5-baner og ingen venteliste.
  const antall5 = n % 4; // 0, 1, 2 eller 3

  const baneStorr = [];
  for (let i = 0; i < antall5; i++) baneStorr.push(5);
  const antall4 = Math.floor(n / 4) - antall5 + (antall5 > 0 ? 0 : 0);
  // Total baner = antall5 + antall4-baner
...
/**
 * Fisher-Yates shuffle — returnerer ny tilfeldig-sortert array.
 * @param {Array} arr
 * @returns {Array}
 */
function blandArray(arr) {
  const a = [...arr];
...
// ════════════════════════════════════════════════════════
// MIX & MATCH — KAMPOPPSETT OG ROTASJON
// ════════════════════════════════════════════════════════

// Kostnad for å pare to spillere som partnere.
// Partner-gjentakelse straffes 10× hardere enn motstander-gjentakelse.
function _mixParCost(a, b, playedWith) {
  return ((playedWith[a.id]?.[b.id] ?? 0) + (playedWith[b.id]?.[a.id] ?? 0)) * 10;
}

// Kostnad for å sette to par mot hverandre.
...
// Velger hvem som spiller og hvem som hviler.
// Prioritet (lav kost = spiller):
//   1. Færrest kamper       — jevner ut spilletid
//   2. Flest hvile-runder   — den som har sittet mest ute spiller nå
//   3. Lenge siden sist ute — unngår to hvile-runder på rad
//   4. Liten støy           — bryter helt like tilfeller tilfeldig
function velgAktiveOgHvilere(spillere, gamesPlayed, sitOutCount, lastSitOutRunde, plasser, runde) {
  ...
  const sortert = spillere.map(s => ({
    ...
    kost: (gamesPlayed[s.id] ?? 0) * 10
        ...
        + Math.random() * 0.5,   // støy < minste vektforskjell
...
// Returnerer { baneOversikt, hviler }.
// baneOversikt[n].spillere = [lag1_s1, lag1_s2, lag2_s1, lag2_s2]  ← kompatibelt med PARTER[0]
function lagMixKampoppsett(spillere, playedWith, playedAgainst, gamesPlayed, sitOutCount, lastSitOutRunde, antallBaner, runde, mp) {
  if (!spillere?.length) return { baneOversikt: [], hviler: [] };

  const poengPerKamp = mp ?? app.poengPerKamp ?? 15;
  const plasser      = antallBaner * 4;

  // Steg 1 - Hvem spiller? Hvem hviler?
  const { aktive, hviler } = velgAktiveOgHvilere(
    spillere, gamesPlayed, sitOutCount, lastSitOutRunde, plasser, runde
  );

  // Trenger minst 4 aktive spillere for en kamp
  if (aktive.length < 4) return { baneOversikt: [], hviler };

  // Steg 2 - Bygg par: greedy, minimiser partner-gjentak.
  ...
  // Trenger minst 2 par for en kamp
  if (par.length < 2) return { baneOversikt: [], hviler };

  // Steg 3 - Sett par mot hverandre: minimiser motstander-gjentak.
...
// Oppdaterer all Mix-statistikk in-place etter en runde.
function oppdaterMixStatistikk(baneOversikt, hvilerDenne, playedWith, playedAgainst, gamesPlayed, sitOutCount, lastSitOutRunde, rundeNr) {
  baneOversikt.forEach(({ spillere: [a, b, c, d] }) => {
    if (!a || !b || !c || !d) return;

    // Hjelp: sikker bi-direksjonell increment
    const incPW = (x, y) => {
    ...
    incPW(a, b); incPW(c, d);           // lagspartnere
    incPA(a, c); incPA(a, d);           // motstandere
    incPA(b, c); incPA(b, d);
...
function hentMixStatistikk(treningData) {
  return {
...
function fordelBanerMix(spillere, antallBaner) {
  return lagMixKampoppsett(spillere, {}, {}, {}, {}, {}, antallBaner, 1, app.poengPerKamp ?? 15);
}


function getParter(antall, erSingel = false) {
 ...
 * @param {Object} par        — fra PARTER eller PARTER_5
 * @param {Array}  spillere   — banens spillere (4 eller 5)
 * @param {number} lag1Poeng
 * @param {number} lag2Poeng
 * @returns {Array} [{ spillerId, poengDelta }]
 */
function beregnPoengForKamp(par, spillere, lag1Poeng, lag2Poeng) {
  const res = [];
  par.lag1.forEach(i => {
...
const app = {
  ...
  // ── SPILLMODUS ──────────────────────────────────────────
  // 'konkurranse' : standard Americano med rating, opprykk/nedrykk og nivåfarger
  // 'mix'         : sosial modus — ingen rating, ingen nivå, tilfeldig lagdeling
  spillModus:         'konkurranse',
};

// ════════════════════════════════════════════════════════
// MODUS-HJELPER
// Én enkelt sannhetskilde for modusbeslutninger.
// Alle steder i koden kaller erMix() i stedet for å sammenligne
// app.spillModus direkte — gjør det enkelt å søke og forstå.
// ════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════
// LÅSE-SYSTEM — forhindrer race conditions og dobbelt-klikk
// ════════════════════════════════════════════════════════

// UI-lås: deaktiverer kritiske knapper mens en operasjon pågår
const UI_LAS_KNAPPER = [
  'neste-runde-knapp',
  'neste-runde-resultat-knapp',
];

function lasUI(melding = 'Systemet jobber…') {
  ...
  // Deaktiver LAGRE-knapp i bekreft-modal
  ...
  // Deaktiver AVSLUTT-knapp i modal
...
// Fail-safe: lås løses automatisk etter 10 sekunder om noe krasjer
...
// Terskler — juster disse for å endre nivågrensene
const NIVAA_TERSKLER = { lav: 950, hoey: 1050 };

/**
 * Returnerer CSS-klassen for et gitt rating-nivå.
 * @param {number} rating
 * @returns {'nivaa-lav'|'nivaa-mid'|'nivaa-hoey'}
 */
function getNivaaKlasse(rating) {
...
/**
 * Returnerer label-objekt for et gitt rating-nivå.
 * @param {number} rating
 * @returns {{ ikon: string, tekst: string, kl: string }}
 */
function getNivaaLabel(rating) {
...
/**
 * Returnerer dot + rating-tall + nivå-label som HTML-streng.
 * @param {number} rating
 * @param {boolean} visLabel — vis nivå-label (default: true)
 * @returns {string}
 */
function getNivaaRatingHTML(rating, visLabel = true) {
  const kl    = getNivaaKlasse(rating);
...
// ════════════════════════════════════════════════════════
// BEFOREUNLOAD — advar ved utilsiktet lukking under aktiv økt
// ════════════════════════════════════════════════════════
...
function naviger(skjerm, retning = null) {
  ...
  // Bunn-nav skjules på skjermer med tilbake-knapp i header, og på hjemskjermen
  ...
  // Vis tilbake-knapp kun på oppsett-skjermen
  ...
  // Vis riktig skjerm med retningsanimasjon
...
// ════════════════════════════════════════════════════════
// SVEIP-NAVIGASJON
// Kun aktiv på hoved-fanene — ikke på underordnede skjermer
// ════════════════════════════════════════════════════════
...
document.addEventListener('touchend', e => {
  ...
  // Ignorer hvis det primært er et vertikalt sveip (scrolling)
  if (Math.abs(dy) > Math.abs(dx)) return;

  // Minimum 60px horisontalt for å telle som sveip
  if (Math.abs(dx) < 60) return;

  // Kun aktiv på hoved-fanene
  if (!SVEIP_FANER.includes(_aktivFane)) return;

  // Ignorer sveip som starter på et input-felt (poengregistrering o.l.)
  const mål = e.target;
  if (mål.tagName === 'INPUT' || mål.tagName === 'TEXTAREA') return;

  const idx = SVEIP_FANER.indexOf(_aktivFane);

  if (dx < 0 && idx < SVEIP_FANER.length - 1) {
    // Sveip venstre → neste fane
    naviger(SVEIP_FANER[idx + 1], 'venstre');
  } else if (dx > 0 && idx > 0) {
    // Sveip høyre → forrige fane
    naviger(SVEIP_FANER[idx - 1], 'hoyre');
  }
}, { passive: true });
...
function lyttPaaSpillere() {
  if (!db) return;
  // Stopp eventuell eksisterende lytter før ny startes
  ...
  // Ikke push til app.lyttere — denne lytteren skal leve hele sesjonen
}

// ════════════════════════════════════════════════════════
// XSS-BESKYTTELSE — escaper HTML-spesialtegn i brukerdata
// ════════════════════════════════════════════════════════
...
function visSpillere() {
  ...
  // 6-spiller singel+dobbel-format: KUN nøyaktig 2 baner OG nøyaktig 6 valgte spillere
  const er6Format = app.antallBaner === 2 && app.valgtIds.size === 6;
  const min       = app.antallBaner * 4; // alltid standard minimum (8 for 2 baner)
  const filtrerte = (app.spillere ?? []).filter(s => (s.navn ?? '').toLowerCase().includes(q));
  const valgtArr  = [...app.valgtIds];

  // Vis hvem som er aktiv vs. ventende basert på fordelingen fordelBaner ville gitt
  const sorterteValgte = valgtArr
    .map(id => (app.spillere ?? []).find(s => s.id === id))
    .filter(Boolean)
    .sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));
  const aktiveIds  = new Set(sorterteValgte.slice(0, min).map(s => s.id));
  const ventendeIds = new Set(sorterteValgte.slice(min).map(s => s.id));

  document.getElementById('spiller-liste').innerHTML = filtrerte.map(s => {
    const erAktiv = aktiveIds.has(s.id);
    const erVente = ventendeIds.has(s.id);
    return lagSpillerHTML(s, erAktiv, erVente);
  }).join('');

  const n      = app.valgtIds.size;
  const nAktiv = Math.min(n, min);
  const nVL    = Math.max(0, n - min);
  document.getElementById('valgt-antall').textContent  = n;
  document.getElementById('aktive-antall').textContent = nAktiv;
  document.getElementById('vl-antall').textContent     = nVL;
  // Start-knapp: 6-spiller-format krev nøyaktig 6, ellers standard minimum
  const effektivMin = er6Format ? 6 : min;
  document.getElementById('start-knapp').disabled = n < effektivMin;
  // Oppdater spiller-info-boksen dynamisk
  const spillerInfoEl = document.getElementById('spiller-info');
  if (spillerInfoEl) {
    if (er6Format) {
      spillerInfoEl.innerHTML = `Nøyaktig <span id="min-antall" style="color:var(--yellow);font-weight:700">6</span> spillere <span style="color:var(--muted)">— 4 dobbel + 2 singel format aktivert</span>`;
    } else {
      spillerInfoEl.innerHTML = `Minst <span id="min-antall" style="color:var(--yellow);font-weight:700">${min}</span> spillere <span style="color:var(--muted)">— ekstra settes på venteliste</span>`;
    }
  } else {
    const minEl = document.getElementById('min-antall');
    if (minEl) minEl.textContent = min;
  }
}
window.visSpillere = visSpillere;

// Debounce på søkefeltet — unngår full DOM-rebuild ved hvert tastetrykk
let _sokTimer = null;
...
function veksleSpiller(id) {
  if (!id) return;
  if (app.valgtIds.has(id)) app.valgtIds.delete(id);
  else app.valgtIds.add(id);
  visSpillere();
}
...
async function startTrening() {
  ...
  if (erMix()) {
    const resultat = fordelBanerMix(valgte, app.antallBaner);
    baneOversikt = resultat.baneOversikt;
    mixHviler    = resultat.hviler ?? [];
  } else {
    baneOversikt = fordelBaner(valgte, app.antallBaner);
  }
  ...
  try {
    ...
    if (erMix()) {
      skrivMixKamper(batch, treningRef.id, 1, baneOversikt, 3, {}, {});
    } else {
...
// Mix & Match — skriv kamper der alle spillere på tvers av baner blandes
// for hver kamp (K1, K2, K3). Resultatet er 3 globale kampoppstillinger
// fordelt tilbake på de samme banene, slik at UI og poengregistrering
// fungerer uendret.
function skrivMixKamper(batch, treningId, rundeNr, baneOversikt, antallKamper, playedWith, playedAgainst) {
  const mp = app.poengPerKamp ?? 15;

  // Samle alle aktive spillere på tvers av baner
  const alleSpillere = baneOversikt.flatMap(b => b.spillere ?? []);
  const antallBaner  = baneOversikt.length;

  // Lag 3 ulike kampoppstillinger (K1, K2, K3)
  for (let kampNr = 1; kampNr <= antallKamper; kampNr++) {
    // Trekk nye lag globalt basert på akkumulert historikk
    const { baneOversikt: nyeBaner } = lagMixKampoppsett(
      alleSpillere,
      playedWith, playedAgainst,
      {}, {}, {},          // gamesPlayed/sitOut ikke relevant her — alle spiller alle kamper
      antallBaner, kampNr, mp
    );

    // Oppdater playedWith/playedAgainst etter denne kampoppstillingen
    // så neste kamp unngår samme partnere/motstandere
    nyeBaner.forEach(({ spillere: [a, b, c, d] }) => {
      if (!a || !b || !c || !d) return;
      const incPW = (x, y) => {
        if (!playedWith[x.id]) playedWith[x.id] = {};
        if (!playedWith[y.id]) playedWith[y.id] = {};
    ...
    // Skriv kampene til Firestore — én kamp per bane med kampNr
    nyeBaner.forEach(bane => {
      const [s1, s2, s3, s4] = bane.spillere;
...
function apnePoenginput(baneNr) {
  ...
  // 6-spiller dobbel-bane: 1 kamp, ikke 3
  const erDobbelBane6 = app.er6SpillerFormat && (bane?.erDobbel === true);
  const parter = erSingelBane ? PARTER_6_SINGEL : (erDobbelBane6 ? PARTER_6_DOBBEL : getParter(bane.spillere.length));
...
async function autolagreKamp(i, l1, l2) {
  ...
  const parter = erSingelLagre ? PARTER_6_SINGEL : (erDobbelLagre6 ? PARTER_6_DOBBEL : getParter(bane?.spillere?.length ?? 4));
  const par    = parter[i];
  if (!par) return;
...
async function visRundeResultat() {
  ...
  // Hent alltid kamper direkte fra Firestore — ikke fra cache som kan være utdatert
  // særlig viktig på siste runde der siste poeng nettopp kan ha blitt lagret
  ...
  nestKnapp.onclick     = erSiste ? visAvsluttModal : () => krevAdmin('Neste kamp', 'Kun administrator kan starte neste kamp. Skriv inn PIN-koden.', bekreftNesteRunde);
  ...
  document.getElementById('resultat-innhold').innerHTML = (app.rangerteBAner ?? []).map(bane => {
    ...
    const _erSingelRes = bane.erSingel ?? false;
    const rader    = bane.rangert.map((s, ri) => {
      ...
      // Mix: ingen nivåfarger i rangeringen
      const nivaaKlRang   = erMix() ? '' : getNivaaKlasse(spillerRating);
    ...
    const bane5merke = er5bane
      ? `<span style="font-size:12px;background:rgba(234,88,12,.15);color:var(--orange);border-radius:4px;padding:2px 7px;font-weight:700">5 SPL</span>`
      : '';
    const singelMerkeRes = bane.erSingel
      ? `<span style="font-size:12px;background:rgba(234,179,8,.15);color:var(--yellow);border-radius:4px;padding:2px 7px;font-weight:700">🏃 SINGEL</span>`
      : (app.er6SpillerFormat ? `<span style="font-size:12px;background:rgba(37,99,235,.15);color:var(--accent2);border-radius:4px;padding:2px 7px;font-weight:700">🎾 DOBBEL</span>` : '');
    // Mix: bane-ikon viser alltid 🎲, konkurranse viser 🏆 / 🔻
    const baneIkon = erMix()
      ? '🎲'
      : (erForst && !bane.erSingel ? '🏆' : erSistB && !app.er6SpillerFormat ? '🔻' : '');
    const baneNummerFarge = bane.erSingel ? 'var(--yellow)' : 'var(--accent)';
    // Mix: vis melding om ny lagblanding på ikke-siste runder
    const mixNesteInfo = (!erSiste && erMix())
      ? `<div class="mix-neste-info">🎲 Lag stokkes om til neste runde</div>`
      : '';
  ...
  try {
    ...
    if (erMix()) {
      ...
      oppdaterMixStatistikk(
        ...
        app.runde   // runden som nettopp er ferdig
      );

      // Inkluder alle — aktive og forrige venteliste — i rotasjonen
      const alleSpillere = [
        ...(app.baneOversikt ?? []).flatMap(b => b.spillere ?? []),
        ...forrigeHvilere,
      ];

      // Lag nytt kampoppsett med oppdaterte statistikker
      const { baneOversikt: nyBaneOversikt, hviler: nyHviler } = lagMixKampoppsett(
        alleSpillere,
        playedWith, playedAgainst, gamesPlayed,
        sitOutCount, lastSitOutRunde,
      ...
      // Mix: globale lag på tvers av baner for hver av de 3 kampene
      skrivMixKamper(batch, app.treningId, nyRunde, nyBaneOversikt, 3, playedWith, playedAgainst);
      await batch.commit();
...
// ════════════════════════════════════════════════════════
// ELO-RATING — oppdateres per kamp, ikke per økt
// ════════════════════════════════════════════════════════
 ...
 * @param {number} ratingA  — gjennomsnittlig rating for lagA
 * @param {number} ratingB  — gjennomsnittlig rating for lagB
 * @returns {number} — tall mellom 0 og 1
 */
 ...
 *
 * @param {Array}  lagA    — [{ id, rating }, { id, rating }]
 * @param {Array}  lagB    — [{ id, rating }, { id, rating }]
 * @param {number} scoreA  — poeng lagA scoret
 * @param {number} scoreB  — poeng lagB scoret
 * @param {number} [K=ELO_K] — valgfri K-faktor
 * @returns {Object} — { [spillerId]: ratingEndring }
 */
function oppdaterRatingForKamp(lagA, lagB, scoreA, scoreB, K = ELO_K) {
  // Validering — returner null-endringer ved ugyldige data
  if (!lagA?.length || !lagB?.length) return {};
  const totalPoeng = (scoreA ?? 0) + (scoreB ?? 0);
  if (totalPoeng <= 0) return {};

  // Lag-rating = snitt av spillernes nåværende rating
  ...
  // Faktisk score — 1 for vinnerlaget, 0 for taperlaget (binær)
  const faktiskA = scoreA > scoreB ? 1 : scoreA < scoreB ? 0 : 0.5;
  const faktiskB = 1 - faktiskA;

  // Ratingendring per spiller
...
/**
 * Kjører Elo-beregning over alle kamper i en økt og akkumulerer endringene.
 * Ratingene oppdateres sekvensielt kamp for kamp slik at hver kamp
 * reflekterer spillernes faktiske rating på det tidspunktet.
 *
 * @param {Array} alleKamper    — alle ferdigspilte kamper for hele økten
 * @param {Array} spillereListe — [{id, rating}] med startrating for alle deltakere
 * @returns {Object} — { [spillerId]: { startRating, nyRating, endring } }
 */
function beregnEloForOkt(alleKamper, spillereListe) {
  // Bygg opp et levende rating-kart som oppdateres for hver kamp
  const ratingNaa = {};
  spillereListe.forEach(s => {
    ratingNaa[s.id] = Number(s.rating) || STARTRATING;
  });

  // Sorter kamper kronologisk: runde → bane → kampNr
  ...
  for (const kamp of sorterte) {
    // Bygg lag-objekter med oppdatert nåværende rating
    // Singel-kamp: lag1_s2 og lag2_s2 er null — filtrer dem ut
    const lagA = [
    ...
    // Oppdater løpende rating så neste kamp bruker fersk verdi
    Object.entries(endringer).forEach(([id, delta]) => {
      if (id in ratingNaa) ratingNaa[id] += delta;
    });
  }

  // Bygg sluttresultat med startrating, ny rating og total endring
  const resultat = {};
  spillereListe.forEach(s => {
...
async function avsluttTreningUI() {
  ...
  try {
    ...
    // Bygg rangerteBAner per bane — bane 1 gir plass 1-4, bane 2 gir plass 5-8 osv.
    // Spillerne i spillerTotaler er allerede summert på tvers av runder.
    // Vi trenger nå å vite hvilken bane hver spiller tilhører (fra siste fullførte runde).
    const sisteFullforteRunde = fullforteRunder[fullforteRunder.length - 1];
    const spillerTilBane = {};
    alleKamper
      .filter(k => k.rundeNr === sisteFullforteRunde)
      .forEach(k => {
        ...
        if (k.hviler_id) spillerTilBane[k.hviler_id] = baneNrInt;
      });

    // Grupper spillere per bane og sorter innad
    const baneGrupper = {};
    Object.values(spillerTotaler).forEach(s => {
      const baneNr = spillerTilBane[s.spillerId] ?? 1;
      if (!baneGrupper[baneNr]) baneGrupper[baneNr] = [];
      baneGrupper[baneNr].push(s);
    });

    const rangerteBAner = Object.keys(baneGrupper)
      .map(Number)
      .sort((a, b) => a - b)
      .map(baneNr => ({
        baneNr,
...
function visMixSluttresultat(data) {
  ...
  document.getElementById('ledertavle').innerHTML = sortert.map((s, i) => {
    ...
    if (sortert.length > 1) {
      ...
      if (s.spillerId === flestKamperId)  utmerkelser.push({ ikon: '💪', tekst: 'Mest aktiv' });
 ...
 * @returns {{ trend: 'up'|'down'|'stable', change: number }}
 */
function beregnTrend(historikk) {
...
async function lastArkiv() {
  ...
  try {
    const snap = await getDocs(
      query(collection(db, SAM.TRENINGER), orderBy('opprettetDato', 'desc'))
    );
    if (laster) laster.style.display = 'none';
...
async function autoAvsluttGamleTreninger() {
  try {
    const snap = await getDocs(
      query(collection(db, SAM.TRENINGER), where('status', '==', 'aktiv'))
    );
...
async function init() {
  ...
  // Ingen aktiv økt funnet — vis hjemskjermen
  try { history.replaceState(null, '', location.pathname); } catch (_) {}
  naviger('hjem');
}
