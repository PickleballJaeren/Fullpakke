...
document.getElementById('sok-inndata')?.addEventListener('input', () => {
  if (!_sokBrukerInput) return;   // ignorer programmatisk endring
...
function veksleSpiller(id) {
  ...
  if (app.valgtIds.has(id)) {
    ...
    app.valgtIds.add(id);
    // Tøm søkefeltet uten å trigge input-event (som ville kalt visSpillere og scrollet)
...
function sorterRangering(stats) {
  ...
  return [...stats]
    .sort((a, b) => b.seire - a.seire || b.diff - a.diff || b.for - a.for || Math.random() - 0.5)
