...
function veksleSpiller(id) {
  ...
  if (app.valgtIds.has(id)) {
    ...
    app.valgtIds.add(id);
    // Tøm søkefeltet — avbryt eventuell debounce-timer
    const sok = document.getElementById('sok-inndata');
    if (sok) sok.value = '';
    clearTimeout(_sokTimer);
  }
  visSpillere();
  // Sett fokus tilbake på søkefeltet etter DOM-oppdatering
  setTimeout(() => document.getElementById('sok-inndata')?.focus(), 50);
}
