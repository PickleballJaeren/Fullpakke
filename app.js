...
async function visRundeResultat() {
  ...
  if (erMix()) {
    // ── MIX: Én samlet rangering på tvers av alle baner ─────────────────
    // Slå sammen alle spillere, sorter etter totalpoeng denne kampen
    const alleSpillere = app.rangerteBAner.flatMap(bane =>
      (bane.rangert ?? []).map(s => ({ ...s, bane: bane.baneNr }))
    ).sort((a, b) => b.diff - a.diff || b.for - a.for || b.seire - a.seire);
