...
function visHjemStatus() {
  ...
  if (harOkt) {
    ...
    if (sub)   sub.textContent   = `Runde ${app.runde} av ${app.maksRunder}`;
...
async function startTrening() {
  ...
  const baneOversikt = erMix()
    ? fordelBanerMix(valgte, app.antallBaner)
    : fordelBaner(valgte, app.antallBaner);
  ...
  // Spillere som ikke fikk plass (skal normalt ikke skje med fordelBaner)
  const baneSpillerIds = new Set(baneOversikt.flatMap(b => b.spillere.map(s => s.id)));
  const venteliste = valgte
    .filter(s => !baneSpillerIds.has(s.id))
...
function oppdaterRundeUI() {
  ...
  // Mix: uformell og sosial runde-indikator-tekst
  document.getElementById('runde-indikator-tekst').textContent = erMix()
    ? `Runde ${app.runde} av ${app.maksRunder} — nye lag, ny sjanse! 🎲`
  ...
  h += `<span class="fremgang-tekst">${app.runde} av ${app.maksRunder} runder</span>`;
  wrap.innerHTML = h;
}
...
function visNesteRundeModal() {
  krevAdmin(
    ...
    () => {
      ...
      if (erMix()) {
        if (tittelEl) tittelEl.textContent = erSiste ? 'Avslutte Mix & Match?' : 'Neste runde?';
        if (tekstEl)  tekstEl.textContent  = erSiste
          ? `Siste runde er ferdig! Vil du se hvem som vant mest? 🎉`
          : `Runde ${app.runde} er ferdig. Klar for nye partnere? 🎲`;
        // Oppdater se-resultater-knappen
...
async function visRundeResultat() {
  ...
  // Mix: sosial sub-header på resultatskjermen
...
async function avsluttTreningUI() {
  ...
  try {
      ...
      kamperIRunde.forEach(kamp => {
        ...
        [...lag1, ...lag2].forEach(spiller => {
          ...
          if (!spillerTotaler[spiller.id]) {
            spillerTotaler[spiller.id] = { spillerId: spiller.id, navn: spiller.navn, seire: 0, for: 0, imot: 0, diff: 0 };
          }
    ...
    // Mix: hent mixGamesPlayed fra Firestore så vi kan vise antall kamper per spiller
    let mixGamesPlayedSnap = {};
    if (erMix()) {
      try {
        const treningSnap = await getDoc(doc(db, SAM.TRENINGER, app.treningId));
        mixGamesPlayedSnap = treningSnap.data()?.mixGamesPlayed ?? {};
      } catch (_) {}
    }
    ...
    app.ratingEndringer = sluttrangering.map(s => {
      if (erMix()) {
        ...
        const antallKamper   = mixGamesPlayedSnap[s.spillerId] ?? 0;
