...
function visSpillere() {
  ...
  const liste = document.getElementById('spiller-liste');
  // Bevar scroll-posisjon — scroll-omrade er det scrollbare elementet på oppsett-skjermen
  const scrollEl = document.getElementById('skjerm-oppsett')?.querySelector('.scroll-omrade');
  const scrollTop = scrollEl?.scrollTop ?? 0;

  liste.innerHTML = filtrerte.map(s => {
    const erAktiv = aktiveIds.has(s.id);
    const erVente = ventendeIds.has(s.id);
    return lagSpillerHTML(s, erAktiv, erVente);
  }).join('');

  // Gjenopprett scroll umiddelbart etter DOM-oppdatering
...
function veksleSpiller(id) {
  ...
  if (app.valgtIds.has(id)) {
    ...
    app.valgtIds.add(id);
    // Tøm søkefeltet og avbryt debounce
    const sok = document.getElementById('sok-inndata');
    if (sok) sok.value = '';
    clearTimeout(_sokTimer);
  }

  // Beregn ny aktiv/ventende-status uten å rebuilde hele listen
  const er6Format = app.antallBaner === 2 && app.valgtIds.size === 6;
  const min = er6Format ? 6 : app.antallBaner * 4;
  const sorterteValgte = [...app.valgtIds]
    .map(sid => (app.spillere ?? []).find(s => s.id === sid))
    .filter(Boolean)
    .sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));
  const aktiveIds   = new Set(sorterteValgte.slice(0, min).map(s => s.id));
  const ventendeIds = new Set(sorterteValgte.slice(min).map(s => s.id));

  // Oppdater kun CSS-klasser og hake-ikon på hvert eksisterende element — ingen scroll-hopp
  document.querySelectorAll('#spiller-liste .spiller-element').forEach(el => {
    const sid      = el.dataset.id;
    const erAktiv  = aktiveIds.has(sid);
    const erVente  = ventendeIds.has(sid);
    const erValgt  = app.valgtIds.has(sid);
    const spiller  = (app.spillere ?? []).find(s => s.id === sid);
    const rating   = spiller?.rating ?? STARTRATING;

    // Sett riktige klasser
    el.className = 'spiller-element'
      + (erAktiv ? ' valgt' : '')
      + (erVente ? ' ventende' : '')
      + (!erAktiv && !erVente && !erMix() ? ' ' + getNivaaKlasse(rating) : '');

    // Oppdater hake
    const hake = el.querySelector('.spiller-hake');
    if (hake) {
      hake.innerHTML = erValgt
        ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>'
        : '';
    }

    // Oppdater VL-merke
    const eksisterendeMerke = el.querySelector('.vl-merke');
    if (erVente && !eksisterendeMerke) {
      const merkeEl = document.createElement('span');
      merkeEl.className = 'vl-merke';
      merkeEl.textContent = 'VL';
      el.insertBefore(merkeEl, hake);
    } else if (!erVente && eksisterendeMerke) {
      eksisterendeMerke.remove();
    }
  });

  // Oppdater teller og start-knapp
  const n = app.valgtIds.size;
  document.getElementById('valgt-antall').textContent  = n;
  ...
  document.getElementById('start-knapp').disabled      = n < min;

  // Bygg listen på nytt kun om søkefeltet var i bruk (filteret endret seg)
  const sok = document.getElementById('sok-inndata');
  if (sok && sok.value === '') {
    // Ingen søk aktiv — listen er allerede korrekt oppdatert in-place
    setTimeout(() => sok.focus(), 50);
  } else {
    // Søk er aktivt — må rebuilde for å vise riktig filtrert resultat
    visSpillere();
  }
}
...
// Lagene er allerede trukket globalt av lagMixKampoppsett og ligger i baneOversikt.spillere
// som [lag1_s1, lag1_s2, lag2_s1, lag2_s2].
function skrivMixKamper(batch, treningId, rundeNr, baneOversikt) {
  baneOversikt.forEach(bane => {
    ...
    batch.set(doc(collection(db, SAM.KAMPER)), {
      ...
      rundeNr,
      kampNr:       1,   // alltid 1 — én kamp per bane per runde i mix
      erSingel:     false,
